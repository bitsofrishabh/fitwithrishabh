...
export default function ClientProfile() {
  ...
  const currentWeight = allWeights.length > 0 ? allWeights[allWeights.length - 1] : client.startWeight;
  ...
  const goalProgress = client.goalWeight ? 
    ((client.startWeight - currentWeight) / (client.startWeight - client.goalWeight)) * 100 : 0;
  ...
  const createWeightChart = () => {
    ...
    const yScale = (weight: number) => 
      height - padding - ((weight - minWeight) * (height - padding * 2)) / (maxWeight - minWeight);

    const pathData = chartData
      .map((d, i) => `${i === 0 ? 'M' : 'L'} ${xScale(d.day)} ${yScale(d.weight)}`)
      .join(' ');
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              ...
              <div className="flex items-center gap-4">
                <span className={`px-3 py-1 rounded-full text-sm font-semibold border ${getStatusStyle(client.status)}`}>
          ...
          >
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900">{weightLoss.toFixed(1)} kg</p>
                <p className="text-sm text-green-600">{weightLossPercentage.toFixed(1)}% reduction</p>
              </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900">
                  {Math.floor((new Date().getTime() - new Date(client.startDate).getTime()) / (1000 * 60 * 60 * 24))}
                </p>
              </div>
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between p-3 bg-green-50 rounded-lg">
                  ...
                  <span className="text-green-600 font-semibold">
                    {currentWeight < 25 ? 'Healthy' : currentWeight < 30 ? 'Overweight' : 'Obese'}
                  </span>
                </div>
                ...
                {goalProgress >= 50 && (
                  <div className="flex items-center justify-between p-3 bg-yellow-50 rounded-lg">
                    ...
                    <span className="text-yellow-600 font-semibold">Halfway There!</span>
                  </div>
                )}
              </div>
            </div>
          ...
          <div className="lg:col-span-2">
            <div className="bg-white rounded-xl shadow-lg p-6 mb-6">
              <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
                <h2 className="text-xl font-bold text-gray-900 mb-4 md:mb-0">Weight Progress</h2>
                <select
                  value={selectedMonth}
              ...
              {chartData.length > 0 && (
                <div className="border-t pt-6">
                  <h3 className="font-semibold mb-4">Daily Weights - {selectedMonth}</h3>
                  <div className="grid grid-cols-7 gap-2 text-sm">
                    {Array.from({ length: getDaysInMonth(selectedMonth) }, (_, i) => i + 1).map(day => {
                      const weight = client.weights[selectedMonth]?.[day.toString()];
                      return (
                        <div
                          key={day}
